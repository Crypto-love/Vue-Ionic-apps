...
export const addCustomerTenant = mutationField('addCustomerTenant', {
  type: 'CustomerTenant',
  args: {
    data: nonNull(
      arg({
  ...
  resolve: async (_parent, { data }, ctx) => {
    ...
    return ctx.prisma.customerTenant.create({
      data: insertCustomertenants
    });
  }
});
