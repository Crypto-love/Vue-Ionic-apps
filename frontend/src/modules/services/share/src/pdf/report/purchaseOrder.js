 ...
 * @param {boolean} base64 - default = false. If true, this function will return Promise<string>
 * @returns {Promise<void> | Promise<string> }
 */
export default async function (payload, fileName, base64 = false) {
  ...
  doc.autoTable({
    ...
    didDrawPage: ({ pageNumber }) => {
      ...
      if (pageNumber === 1) {
        ...
        createUpperDescription(doc, { text: 'Order Date', y: 40 }, { text: payload.po_date || '-', y: 44 });
        ...
        createUpperDescription(
          doc,
          { text: 'Order Number', y: 60 },
          { text: payload.order_number || '-', y: 64 }
        );
        createUpperDescription(doc, { text: 'GST', y: 70 }, { text: '201724120K', y: 74 });
      }
    }
  });
  ...
  doc.text(
    'Please contact Nicholas at +65 9017 3814 or NICHOLAS@THETREEDOTS.COM for all payment related matters.',
    rect1InfoX,
    rect1InfoY,
    {
      maxWidth: rect1Width - 5
