    ...
    <div class="d_header q-mb-md">
      <div class="row">
        <div class="col-xs-12 col-md-4">
          <div class="text-title text-h6">Hubs</div>
        </div>
          ...
          >
            <template #additional-btn>
              <q-btn
                ...
                @click="
                  showPrompt('Download Confirmation', 'You will download all hubs data as CSV', onDownloadCSV)
                "
                >Download CSV</q-btn
              >
            </template>
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td :props="props" key="actions">
            ...
            >
              <q-tooltip>Tag sales person to this hub</q-tooltip>
            </q-icon>
          </q-td>
        ...
        <template v-if="props.expand">
          ...
          <q-tr v-else>
            <q-td colspan="100%">This company has no branches</q-td>
          </q-tr>
        </template>
      </template>
    </q-table>
    <q-dialog v-model="showForm">
      <my-form :id="selectedItem" @close="onFormClosed" />
    </q-dialog>
...
import MyForm from './hubs/Form.vue';
...
export default {
  components: {
    ...
    PersonCard,
    TableTopRight,
    TagSalesPersonForm: () => import('./hubs/TagSalesPersonForm.vue')
  },
  data() {
    return {
      showForm: false,
  ...
  methods: {
    sendStatementConfirm(data) {
      this.showPrompt('Confirmation', 'You will send statement to this hub', () => this.sendStatement(data));
    },
    ...
    async getData() {
      ...
      const res = await Api.get(
        'v_hub_page_dashboard',
        `customer_type_id = 2 and hub = 1 and tenant_id = ${this.$store.state.tenant_id}`,
        null,
        'id DESC'
      );
    ...
    async getDataById(id) {
      const { data } = await Api.get(
        'v_hub_page_dashboard',
        `id = ${id} and hub = 1 and tenant_id = ${this.$store.state.tenant_id}`
      );
      const d = {
        ...data[0],
    ...
    onShowTagSalesForm(id, parentId = null) {
      this.selectedItem = {
        id,
        parentId
      };
    ...
    async onDownloadCSV() {
      try {
        ...
        link.setAttribute('download', `${this.$dayjs().format('YYYYMMDD')}_All_Hubs.csv`);
