<template>
  <form-dialog :title="isEditMode ? `Edit Tenant` : `Add New Tenant`" class="modal-xl">
          ...
          <div class="row justify-between q-col-gutter-md">
            ...
            <div class="col-12">
              <q-input
                ...
                outlined
                autogrow
                v-model="formData.tax_registration_number"
            ...
            <div class="col-6">
              ...
              <!-- <q-select
                ...
                :options="days"
                label="Delivery Day"
                :rules="[(v) => !!v || 'Profile cannot be empty']" class="form-group mb-2"
              >
...
export default {
  ...
  computed: {
    isEditMode() {
      return this.item !== null || this.formData.id !== null;
    },
    custName() {
  ...
  async mounted() {
    ...
    this.tenants.forEach((v) => {
      if (this.isEditMode && v.id != this.item.id && v.alias_name != null)
        this.existingUniqueId.push(v.alias_name);
      else if (!this.isEditMode && v.alias_name != null) this.existingUniqueId.push(v.alias_name);
    });
  },
  methods: {
    async getTenantDetail() {
      try {
         ...
         */
        const { addresses, persons, ...data } = this.item;
    ...
    async getGroup() {
      try {
        const res = await this.getAdditionData('groups', true);
    ...
    async getCustomerType() {
      try {
        const res = await this.getAdditionData('collection_types', true);
        this.collectionTypes = [...res?.data?.getCollectionTypes];
      } catch (error) {
        Notice.fail(error.message);
      }
    },
    async getCollectionType() {
      try {
        const res = await this.getAdditionData('customer_types', true);
        this.customerTypes = res?.data?.getCustomerTypes;
      } catch (error) {
        Notice.fail(error.message);
      }
    },
    ...
    async onSubmit() {
      ...
      if (this.isEditMode) {
        ...
        const newData = data.addSupplierDashboardAdmin;
    ...
    async add() {
      ...
      //remove tenant id when create new tenant
      ...
      if (res.data.addSupplierDashboardAdmin) {
        Notice.ok('New Tenant added');
      } else {
    ...
    async edit() {
      ...
      const res = await this.updateNewSupplier(payload, false);

      Notice.ok(res.data.updateSupplierDashboardAdmin ? 'Tenant data updated' : '');
    ...
    async updateNewSupplier(data, newTenant = true) {
      try {
        return await this.$apollo
          .mutate({
            mutation: newTenant ? addNewSupplierDashboardAdmin : updateNewSupplierDashboardAdmin,
            variables: {
              data: data
            }
          })
    ...
    async getAdditionData(dataType, active) {
      try {
        return await this.$apollo
          .query({
            query:
              dataType === 'groups'
                ? getGroups
                : dataType === 'customer_types'
                ? getCustomerTypes
                : dataType === 'collection_types'
                ? getCollectionTypes
                : null,
            variables: {
              active: active
