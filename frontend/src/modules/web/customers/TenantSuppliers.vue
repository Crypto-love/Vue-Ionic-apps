    ...
    <div class="d_header q-mb-md">
      <div class="row">
        <div class="col-xs-12 col-md-4">
          <div class="text-title text-h6">Tenant Suppliers</div>
        </div>
...
import MyForm from './suppliers/TenantFormSupliers.vue';
...
export default {
  ...
  methods: {
    ...
    async getDataById(id) {
      let tenant_id = this.$store.state.tenant_id;
      const { data } = await Api.get('v_customer_page_dashboard', `id = ${id} and tenant_id = ${tenant_id}`);
      const d = {
        ...data[0],
