<template>
  <form-card title="Invoices Related to This Order">
      ...
      <template v-slot:body-cell-action="props">
        <q-td :props="props" class="q-gutter-x-xs">
          <q-btn
            ...
            no-caps
            @click="
              getInvoices(props.row.invoice_id);
              viewInvoice = true;
            "
          />
        </q-td>
    ...
    <q-dialog v-model="viewInvoice">
      <view-invoice :data="selectedItem" v-if="viewInvoice" />
    </q-dialog>
...
export default {
  ...
  methods: {
    ...
    async getInvoices(id) {
      const { data } = await Api.get('v_invoices', `id=${id}`);
      this.selectedItem = data.length > 0 ? data[0] : {};
    }
  }
};
</script>
