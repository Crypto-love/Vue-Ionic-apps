<template>
  <form-dialog title="Summary Order" class="modal-md">
    <q-card-section>
    ...
    async getOrderItem() {
      try {
        const res = await Api.get(
          ...
          `delivery_date = "${this.dateFormatted}" and tenant_id=${this.credentials.tenant_id}`,
          'distinct id,name',
          'name'
        );
        this.items = res.data;
    ...
    async checkDetail() {
      try {
        const res = await Api.get(
          ...
          `delivery_date = "${this.dateFormatted}" and id = ${this.selectedItems} and tenant_id=${this.credentials.tenant_id}`,
