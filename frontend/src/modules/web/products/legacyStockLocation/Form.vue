...
export default {
  ...
  data() {
    return {
      creditPayment: { label: 'COD', value: 0 },
      paymentType: { label: 'Bank Transfer', value: 0 },
      paymentMethod: false,
      submitLoading: false,
      addresses: [],
      branches: [],
      ...
      dueDateRules: [
        (v) => !!v || 'Please type something',
        (v) => (v && isNumeric(v)) || 'Please use numeric character',
        (v) => (v && Number(v) > 0) || 'Should be more than 0',
        (v) => (v && Number(v) != 30) || 'use " 30 Days option"',
        (v) => (v && Number(v) <= 60) || 'Should be less than 60'
      ],
      minimumOrderRules: [
        (v) => isNumeric(v) || 'Please use numeric character',
    ...
    async add() {
      ...
      if (this.isThirdPartyAllowed && !this.isABranch && !this.isASupplier) {
        ...
        if (this.xeroTenantId) {
          /** There's a chance that this.formData.account_number will be replaced after we execute this.createXeroContact() */
