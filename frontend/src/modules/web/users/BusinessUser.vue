    ...
    <div class="d_header q-mb-md q-mt-lg">
      <div class="row">
        <div class="col-xs-12 col-md-12">
          <div class="text-title text-subtitle2 text-weight-bold text-uppercase">Business Users</div>
        </div>
      </div>
    </div>
    <q-table
...
import { getSupplierUsers, updateSupplierUser, deleteSupplierUsers } from 'treeGQL';
...
export default {
  ...
  methods: {
    ...
    gotoDetail(row) {
      if (row.id != null) {
        this.$router.push({ path: `/main/users/supplier_user/` + row.id + `/2` });
      }
    },
    gotoEdit(row) {
      this.$router.push({ path: `/main/users/supplier_user/` + row.id + '/2/edit' });
    },
    ...
    gotoNew() {
      this.$router.push({ path: `/main/users/new/2` });
    },
    replaceStar(str) {
