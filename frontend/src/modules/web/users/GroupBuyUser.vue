    ...
    <div class="d_header q-mb-md q-mt-lg">
      <div class="row">
        <div class="col-xs-12 col-md-12">
          <div class="text-title text-subtitle2 text-weight-bold text-uppercase">Group Buy Users</div>
        </div>
      </div>
    </div>
    <q-table
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        const response = await this.$apollo
          .query({
            ...
            variables: {
              buyerType: 2,
              userType: [6, 11], //need to also showing hosts user here
              page: page,
              perPage: perPage,
              keyword: keyword,
              active:
    ...
    gotoDetail(row) {
      if (row.id != null) {
        this.$router.push({ path: `/main/users/supplier_user/` + row.id + `/1` });
      }
    },
    gotoEdit(row) {
      if (row.id != null) {
        this.$router.push({ path: `/main/users/supplier_user/` + row.id + '/1/edit' });
      }
    },
    ...
    gotoNew() {
      this.$router.push({ path: `/main/users/new/1` });
    },
    replaceStar(str) {
