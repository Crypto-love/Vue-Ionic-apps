<template>
  <form-card :title="item ? 'Edit Tenant' : 'Add Tenant'" class="form-card">
  ...
  methods: {
    ...
    async getCustomers() {
      const res = await this.$api.get(
        'customers',
        'customer_type_id = 3 AND id NOT IN (SELECT customer_id FROM v_user_customer_name WHERE user_type_id = 2)'
      );
