<template>
  <form-card :title="isEditMode ? 'Edit User' : 'Add New Users Tenant'">
      ...
      <div class="row">
        ...
        <div class="col-xs-12 col-md-8 q-pl-md-md q-mt-xs-lg q-mt-md-none">
          ...
          <div class="q-mt-lg">
            <customer-table
              :user-id="formData.id"
              :user-type-id="formData.user_type_id"
              :disabled="!isEditMode"
            />
          </div>
        </div>
...
<script>
import { Api, Notice, fileToBase64, cipher, decipher, isEmailValid, isNumeric, isValidName } from 'services';
  ...
  methods: {
    ...
    async getUsersDetail() {
      try {
        ...
        this.selectedCountries = this.countries.find((v) => v.id === data.country_id);
    ...
    async getUserType() {
      try {
        const res = await Api.get('user_types', `id in (4, 5, 6, 7, 8, 9, 11, 12)`);
    ...
    async addUserCustomer() {
      /** Tag new user to tenant */
      await Api.add('user_customers', {
        user_id: this.formData.id,
        customer_id: this.$store.state.tenant_customer_id
      });
    }
  }
};
